...
MATCHER_P(IsEqualMatrix, rmatrix, "")
{
		...
		row < rmatrix.GetRows(); row++)
	{
		...
		{
			if(std::abs(arg.Get(row, col) - rmatrix.Get(row, col)) > 1e-6)
			{
				return false;
			}
		}
...
TEST_F(MatrixConstructionTest, MoveConstructorTestSuccessful_1)
{
		...
		row < moved.GetRows(); row++)
	{
		for(std::size_t col = 0;
			...
			col++, ++num)
		{
			ASSERT_EQ(moved.Get(row, col), num);
		}
	}
}
...
{
	// 10 - is largest number when calculation takes adequate time(less then second)
	Matrix toGetDeterminant{ 10 };
